...
plt.fill_between(
    density1[0], density1[1], color="none", hatch="///", edgecolor="black", alpha=0.5
)
plt.fill_between(
    density2[0], density2[1], color="none", hatch="\\\\\\", edgecolor="green", alpha=0.5
)
...
plt.text(
    800, 0.00048, "Mean = {:.0f}".format((df_l1_cleaned.lambda_t.mean())), color="green"
)
...
plt.text(
    5000,
    0.00048,
    "Mean = {:.0f}".format((df_l1_cleaned.lambda_t_raw.mean())),
    color="black",
)
...
def corrfunc(x, y, ax=None, loc=[0.95, 0.25], **kws):
...
def plot_regression(ax, x, y, fit_type="linear", color="red", loc=[0.05, 0.9]):
    ...
    if fit_type == "log-log":
        ...
        ax.text(
            ...
            color=color,
            bbox=dict(
                facecolor="white", alpha=0.2, edgecolor="none", boxstyle="round,pad=1"
            ),
        )
...
sns.kdeplot(
    data=df_l1_cleaned, x="Re_tb", ax=ax_marg_x_0, log_scale=True, color="black"
)
sns.kdeplot(
    data=df_l1_cleaned, x="Re_di", ax=ax_marg_x_1, log_scale=True, color="black"
)
sns.kdeplot(
    data=df_l1_cleaned, x="Re_lt", ax=ax_marg_x_2, log_scale=True, color="black"
)
...
colors = {"cleaned": current_palette[0], "outliers": "lightgray"}
...
corrfunc(df_l1_cleaned["Re_tb"], df_l1_cleaned["Re_lt"], ax_joint_0, loc=(0.95, 0.4))
...
sns.histplot(
    ax=ax_joint_0, data=df_l1_cleaned, x="lambda_c_fit", y="lambda_c_e", log_scale=True
)
...
sns.histplot(
    ax=ax_joint_1, data=df_l1_cleaned, x="lambda_c_e", y="lambda_c_int", log_scale=True
)
...
for ax in [ax_marg_x_0, ax_marg_x_1, ax_marg_x_2]:
    ...
    ax.set_xlim(
        1e5, 1e7
    )  # Annoying adjustment of xlim upper needed to make the gridlines look nice
    ax.axis("off")
